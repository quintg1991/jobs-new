...
// START OF ALL OF THE FUN STUFF========================================================================================================================================================================
  ...
  if(isset($_GET['projID']))
  {
    ...
    $_jobNoteSql = "SELECT `note`.`noteID` AS `noteID`, `note`.`projID` AS `projID`, `note`.`note` AS `note`, `note`.`dateTime` AS `dateTime` FROM `note` WHERE `note`.`projID`=:projID AND `note`.`deleted` != '1'";
  ...
  // FILE UPLOAD PROCESSING & QUERY=================================================================
  if(isset($_GET['projID']))
  {
    if(isset($_POST['fileSubmit']))
    {
      $_targetDir = $_SERVER['DOCUMENT_ROOT'] . '/jobs-new/uploads/';
      $_targetFile = $_targetDir . basename($_FILES['jobFile']['name']);
      $_uploadOK = 1;
      $uploadFileType = pathinfo($_targetFile, PATHINFO_EXTENSION);

      // Get the different parts of the file name
      $path_parts = pathinfo($_targetFile);
      ...
      elseif($uploadFileType != 'jpg' && $uploadFileType != 'pgn' && $uploadFileType != 'jpeg' && $uploadFileType != 'doc' && $uploadFileType != 'docx' && $uploadFileType != 'txt' && $uploadFileType != 'xls' && $uploadFileType != 'pdf')
      {
      ...
      elseif(move_uploaded_file($_FILES['jobFile']['tmp_name'], $_targetFile))
      {
        //Enter the upload information into the DB
        $_fileInfoSql = "INSERT INTO `file` (`file`.`projID`, `file`.`fileName`, `file`.`fileType`, `file`.`uploadedBy`, `file`.`dateTime`) VALUES (:projID, :fileName, :fileType, :uploadedBy, :dateInsert)";
        $_fileInfoStmnt = $_pdo->prepare($_fileInfoSql);
        ...
        $_params = 
          ...
          (
            ...
            ':uploadedBy' => $_SESSION['initials'],
            ':dateInsert' => date('Y-m-d H:i:s'),
          );

        if($_fileInfoStmnt->execute($_params))
        {
          header("Refresh:0");
        }
        else
        {
          echo "made it to the insert else statment";
        }
      }
    }
    else
    {
      $_fileInfoSql = "SELECT `file`.`fileID` AS `fileID`, `file`.`projID` AS `projID`, `file`.`fileName` AS `fileName`, `file`.`fileType` AS `fileType`, `file`.`uploadedBy` AS `uploadedBy`, `file`.`dateTime` AS `dateTime` FROM `file` WHERE `file`.`projID`=:projID";
          ...
          if($_fileInfoStmnt->execute($_params))
          {
            $file = $_fileInfoStmnt->fetchAll(PDO::FETCH_ASSOC);
            // var_dump($file);
          }
          else
          {
...
// END FILE UPLOAD PROCESSING & QUERY=============================================================
?>
      ...
      <div class="row">
        <div class="col">
          ...
          <form method="POST" enctype="multipart/form-data">
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">                        
                          <input class="form-control" id="projName" type="text" name="projName" required="required" <?php echo isset($_GET['projID']) ? isset($job['projName']) ? 'value="' .$job['projName']. '"' : 'value=" "' : 'value=""' ?> />
                        </div>
                      </div>
                    </td>
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          <select class="form-control" id="projStatus" name="projStatus">
                            <option value="">-</option>
                            <option value="assigned" <?php echo isset($_GET['projID']) ? ($job['projStatus'] == 'Assigned' ? 'selected="selected"' : '') : ''; ?>>Assigned</option>
                            <option value="in progress" <?php echo isset($_GET['projID']) ? ($job['projStatus'] == 'In Progress' ? 'selected="selected"' : '') : ''; ?>>In Progress</option>
                            <option value="proof out" <?php echo isset($_GET['projID']) ? ($job['projStatus'] == 'Proof Out' ? 'selected="selected"' : '') : ''; ?>>Proof Out</option>
                            <option value="finished" <?php echo isset($_GET['projID']) ? ($job['projStatus'] == 'Finished' ? 'selected="selected"' : '') : ''; ?>>Finished</option>
                            <option value="unassigned" <?php echo isset($_GET['projID']) ? ($job['projStatus'] == 'Unassigned' ? 'selected="selected"' : '') : ''; ?>>Unassigned</option>
                            <option value="archive" <?php echo isset($_GET['projID']) ? ($job['projStatus'] == 'Archive' ? 'selected="selected"' : '') : ''; ?>>Archive</option>
                          </select>
                        </div>
                      </div>
                    </td>
                ...
                <tbody>
                  <tr>
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          <input class="form-control" id="dateDue" type="date" name="dateDue" required="required" <?php echo isset($_GET['projID']) ? isset($job['dateDue']) && $job['dateDue'] != '1969-12-31' ? 'value="' .$job['dateDue']. '"' : 'value=""' : 'value=""' ?>>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          <input class="form-control" id="dateComplete" type="date" name="dateComplete" <?php echo isset($_GET['projID']) ? isset($job['dateComplete']) && $job['dateComplete'] != '1969-12-31' ? 'value="' .$job['dateComplete']. '"' : 'value=" "' : 'value=""' ?>>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          <input class="form-control" id="dateRequested" type="date" name="dateRequested" <?php echo isset($_GET['projID']) ? isset($job['dateRequested']) && $job['dateRequested'] != '1969-12-31' ? 'value="' .$job['dateRequested']. '"' : 'value=" "' : 'value=""' ?>>
                        </div>
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      <div class="form-group">
                        <input class="form-control" id="yearNum" type="text" name="" <?php echo isset($job['yearNum']) ? 'value="' .$job['yearNum']. '" readonly="readonly" ' : 'disabled="disabled"' ?>
                         >
                      </div>
                    </td>
                  </tr>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          <input class="form-control" id="dateProof1" type="date" name="dateProof1" <?php echo isset($job['dateProof1']) && $job['dateProof1'] != '1969-12-31' ? 'value="' .$job['dateProof1']. '"' : 'value=""' ?> placeholder="">
                        </div>
                      </div>
                    </td>
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          <input class="form-control" id="dateProof2" type="date" name="dateProof2" <?php echo isset($job['dateProof2']) && $job['dateProof2'] != '1969-12-31' ? 'value="' .$job['dateProof2']. '"' : 'value=""' ?>>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          <input class="form-control" id="dateProof3" type="date" name="dateProof3" <?php echo isset($job['dateProof3']) && $job['dateProof3'] != '1969-12-31' ? 'value="' .$job['dateProof3']. '"' : 'value=""' ?>>
                        </div>
                      </div>
                    </td>
                    <td>
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <div class="form-check form-check-inline">
                        <label class="form-check-label">
                          <input type="checkbox" id="homeArt" name="homeArt" value="1" <?php echo isset($_GET['projID']) ? ($job['homeArt'] == '1' ? 'checked="checked"' : '') : '' ?> />
                        </label>
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <?php
                      ...
                      if(isset($_GET['projID']))
                      {
                        ...
                        {
                          echo '<tr>';
                          echo '<td><strong>' . date("Y-m-d", strtotime($f['dateTime'])) . '</strong></td>';
                          echo '<td>' . $f['fileName'] . '</td>';
                          echo '<td>' . $f['uploadedBy']. '</td>';
                          echo '<td><a class="btn btn-default" href="download.php?fileID=' . $f['fileID'] . '"><span class="fa fa-download"></a><a class="btn btn-default" href="delete.php?fileID=' . $f['fileID'] . '"><span class="fa fa-trash-o"></a></td>';
                        }
                      }
                    ?>
            ...
            <div class="input-group">
              <input class="form-control" id="jobFile" type="file" name="jobFile" /> 
              <span class="input-group-btn">
                <button class="btn btn-secondary" id="fileSubmit" name="fileSubmit" type="submit">
                  Upload File
                </button>
              </span>            
            </div>
                  ...
                  <tr>
                    <td>
                      <div class="form-check form-check-inline">
                        ...
                        <?php echo ($job['projRush'] == 1) ? '<p class="fa fa-check fa-2x"></p>' : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php if(($job['dateDue'] == '0000-00-00') || ($job['dateDue'] == '1969-12-31'))
                            {
                              echo 'If you are reading this, there is an error with the job.';
                            }
                            else
                            {
                              echo $job['dateDue'];
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php if(($job['dateComplete'] == '0000-00-00') || ($job['dateComplete'] == '1969-12-31'))
                            {
                              echo '';
                            }
                            else
                            {
                              echo $job['dateComplete'];
                            }
                          ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php if(($job['dateRequested'] == '0000-00-00') || ($job['dateRequested'] == '1969-12-31'))
                            {
                              echo '';
                            }
                            else
                            {
                              echo $job['dateRequested'];
                            }
                          ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                            ...
                            <?php echo ($job['projUserID'] != '') ? $job['projUserID'] : 'No primary user assigned';
                            ?>

                            <br />
                            <br />

                            <?php echo ($job['projUserID2'] != '') ? $job['projUserID2'] : '';
                            ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo (isset($job['reqName'])) ? $job['reqName'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                            ...
                            <?php echo ($job['projClass'] != '') ? $job['projClass'] : '';
                            ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                            ...
                            <?php echo ($job['projPrinting'] != '') ? $job['projPrinting'] : '';
                            ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                            ...
                            <?php echo ($job['projType'] != '') ? $job['projType'] : '';
                            ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo ($job['reqPhone'] != '') ? $job['reqPhone'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php if(($job['dateProof1'] == '0000-00-00') || ($job['dateProof1'] == '1969-12-31'))
                            {
                              echo '';
                            }
                            else
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php if(($job['dateProof2'] == '0000-00-00') || ($job['dateProof2'] == '1969-12-31'))
                            {
                              echo '';
                            }
                            else
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php if(($job['dateProof3'] == '0000-00-00') || ($job['dateProof3'] == '1969-12-31'))
                            {
                              echo '';
                            }
                            else
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo isset($job['proofOut']) ? $job['proofOut'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo isset($job['reqEmail']) ? $job['reqEmail'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo isset($job['detailColorCover']) ? $job['detailColorCover'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                         ...
                         <?php echo isset($job['detailColorInside']) ? $job['detailColorInside'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo isset($job['detailSize']) ? $job['detailSize'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo isset($job['detailPages']) ? $job['detailPages'] : ''; ?>
                  ...
                  <tr>
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo isset($job['copyNeeded']) ? $job['copyNeeded'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo ($job['copyReceived'] == 1) ? '<p class="fa fa-check fa-2x"></p>' : ''; ?>
                          <?php echo isset($job['copySource']) ? $job['copySource'] : ''; ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo isset($job['photographyNeeded']) ? $job['photographyNeeded'] : '' ?>
                    ...
                    <td>
                      <div class="container-responsive">
                        <div class="form-group">
                          ...
                          <?php echo ($job['photographyReceived'] == 1) ? '<p class="fa fa-check fa-2x"></p>' : ''; ?>
                          <?php echo isset($job['photographySource']) ? $job['photographySource'] : ''; ?>
                    ...
                    <td>
                      <div class="form-check form-check-inline">
                        ...
                        <?php echo ($job['homeArt'] == 1) ? '<p class="fa fa-check fa-2x"></p>' : ''; ?>
